...
See [the reference guide](https://docs.irenmu.com/api/recorder.md) for details.
...
See [the reference guide](https://docs.irenmu.com/api/video.md) for details.
...
See [the reference guide](https://docs.irenmu.com/api/download.md) for details.

### 软件配置

当涉及配置修改，查询时使用

See [the reference guide](https://docs.irenmu.com/api/config.md) for details.
...
如果用户提供了url，类似`https://www.bilibili.com/video/BV1YDTq6EEBK`,使用接口解析后调用下载。

#### 视频解析

调用 `/video/parse` 进行解析，获取到下载链接后进行下载，如果存在多P的情况，且用户没有指定，需要列出供用户进行选项，文件命名采用视频标题，如果存在多P，使用分P标题命名。
